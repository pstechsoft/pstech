...
var i,
	...
	funescape = function( escape, nonHex ) {
		...
		return nonHex ?
			...
			// Replace a hexadecimal escape sequence with the encoded Unicode
			// code point
			// Support: IE <=11+
	...
	if ( !seed ) {
		...
		if ( documentIsHTML ) {
			...
			if ( support.qsa &&
				...
				// The technique has to be used as well when a leading
				// combinator is used
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	// Support: IE 8 - 11+, Edge 12 - 18+, Chrome <=16 - 25 only, Firefox <=3.6
	// - 31 only,
	...
	if ( ( support.qsa = rnative.test( document.querySelectorAll ) ) ) {
		...
		assert( function( el ) {
			...
			// cases.
			// Adding a temporary attribute to the document before the selection
			// works
			// around the issue.
...
( function() {
	...
	// Support: IE <=9 only
	// IE <=9 replaces <option> tags with their contents when inserted outside
	// of
	// the select element.
